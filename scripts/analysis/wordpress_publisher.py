...
DEPRECATED for pipeline use — prefer the canonical ``scripts/wordpress/wp_publisher.py``
(see scripts/analysis/README.md). Kept for ad-hoc/analysis flows only. Term matching
here MUST stay HTML-entity-aware (see ``_norm_term_name``): WordPress returns term
names entity-encoded (``Ash &amp; Fur``), and raw ``.lower()`` comparison is exactly
the bug that minted the 2026-06 duplicate top-level categories.
"""
...
class WordPressPublisher:
    ...
    def get_categories(self) -> Dict[str, int]:
        ...
        while True:
            ...
            if not items:
                break
            for cat in items:
                categories[_norm_term_name(cat['name'])] = cat['id']
            page += 1
    ...
    def get_tags(self) -> Dict[str, int]:
        ...
        while True:
            ...
            if not items:
                break
            for tag in items:
                tags[_norm_term_name(tag['name'])] = tag['id']
            page += 1
    ...
    def get_or_create_category(self, name: str) -> int:
        ...
        name_lower = _norm_term_name(name)
