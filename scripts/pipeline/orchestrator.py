...
STAGES: list[Stage] = [
    ...
    Stage("research", "research", "LLM",
          ...
          dispatch_prompt="Run full Phase Research for keyword '{primary_keyword}' on project {project_slug}. Use the Python scripts (they add key-rotation + retry + cost-ledger + cache; MCP tools are fallback only). ATTRIBUTION (mandatory): EVERY tavily_search / tavily_extract / serpapi_query / tavily_research call carries `--task-id {task_id}` — a ledger row without it breaks per-article cost attribution and poisons batch actuals (2026-08-17 audit). MANDATORY: (1) Start with `python -m scripts.fetch.tavily_research \"...{primary_keyword}...\" --model pro --task-id {task_id}` for comprehensive deep research → save to research/deep-research.json. (2) Then run 4-5 `python -m scripts.fetch.tavily_search \"...\" --depth advanced --task-id {task_id}` calls (NEVER basic). (3) Use `python -m scripts.fetch.tavily_extract --task-id {task_id}` for top competitor pages. (4) Run `python -m scripts.fetch.crossref_lookup` for academic sources. (5) SERP GROUND TRUTH (non-negotiable — Tavily inference under-detects features; the 07-01 batch missed a live AI Overview): run `python -m scripts.fetch.serpapi_query --engine google --q \"{primary_keyword}\" --gl us --hl en --task-id {task_id} --json` and derive `serp_features[]` + `_serp_features_detail{{}}` + `ai_overview_present` from its STRUCTURED response (ads/answer_box/related_questions/inline_videos/ai_overview/local_results/inline_images/discussions_and_forums keys), never from memory. Consolidate ALL findings into workspace research.json via the Write tool (hook-validated against the schema) — NEVER by authoring an ad-hoc workspace script (a Bash-side write bypasses write-time schema validation) — using the CANONICAL schema keys (primary_keyword / intent as an ENUM STRING / competitor_titles / paa / semantic_clusters / content_gaps) and the CANONICAL serp_features vocabulary — map raw SerpApi keys before writing: ads→paid_ads, related_questions→paa_box, local_results+local_map→local_pack, inline_videos→video_carousel, answer_box→featured_snippet, sitelinks→site_links (full map: scripts/validate/research_contract.py::SERPAPI_TO_CANONICAL). FINAL CONSOLIDATION STEP (mandatory, v3.36.0): run `python -m scripts.validate.research_contract --workspace {task_id} --fix --json` and confirm passed:true — the runner hard-rejects a drifted shape (it will also self-heal known variants once, but an intentional canonical write always beats the safety net). The deep-research pro call is NON-NEGOTIABLE. If a script exhausts all keys and raises, fall back to the matching mcp__tavily__* tool for that one call."),
...
# is surfaced by pre_publish_gate's gate_freshness check instead: review.json
# older than draft.md is a mandatory FAIL there (unreviewed content, 2026-08-17);
# fact-check/humanizer/quality staleness stays WARN (operator's trade-off).
_FRESHNESS_VS_DRAFT = {
    "quality.json",
    "render-lint.json",
...
def _render_dispatch_prompt(dispatch_prompt: str, task_id: str, state: dict) -> str:
    ...
    # Image-count placeholders (2026-08-17): the designer prompt used to hardcode
    # "design 4 image prompts" while the outline flags brief.image_count−1 slots —
    # two spellings of one fact. scripts/_core/image_policy.py is the resolver.
    if "{image_count}" in prompt or "{inline_image_count}" in prompt:
        from scripts._core.image_policy import inline_image_count, resolve_image_count
        prompt = prompt.replace("{image_count}", str(resolve_image_count(state)))
        prompt = prompt.replace("{inline_image_count}", str(inline_image_count(state)))
