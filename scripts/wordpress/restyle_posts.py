...
def restyle_content(raw: str, css_min: str, site_slug: str = "") -> tuple[str, dict]:
    ...
    if site_slug:
        ...
        # On a token project, any surviving legacy name is a leak — count it so
        # a dry-run surfaces the defect instead of hiding it (Rule 14).
        stats["legacy_class_leaks"] = len(re.findall(r'class="[^"]*(?<![\w-])xr-', out)) + len(
            re.findall(r"\.xr-[a-z0-9-]+\s*\{", out)
        ) if style_tokens.enabled(site_slug) else 0
...
def main() -> int:
    ...
    print(json.dumps(summary, indent=2) if args.json else
          f"{summary['mode']}: {summary['changed']} changed, {summary['applied']} applied "
          f"of {len(results)} post(s)")
    return 0
