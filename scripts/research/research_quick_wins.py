...
from opportunity_scorer import OpportunityScorer, OpportunityType
...
def main():
    ...
    quick_wins = gsc.get_quick_wins(
        days=30, position_min=11, position_max=20, min_impressions=50
    )
    ...
    for i, kw in enumerate(quick_wins[:10], 1):
        ...
        print(f"CTR: {ctr:.2f}%")
        print(
            f"Commercial Intent: {kw['commercial_intent_category']} ({kw['commercial_intent']}/3.0)"
        )
        ...
        if dfs:
            try:
                ...
                ranking = get_first_ranking(rankings)
                if ranking and ranking.get("position"):
                    dfs_position = ranking["position"]
                    dfs_url = ranking.get("url")
                    search_volume = ranking.get("search_volume")
                    difficulty = ranking.get("difficulty")
        ...
        if ga and kw.get("ranking_url"):
            try:
                ...
                if page_data and "pageviews" in page_data:
                    ...
                    print(
                        f"  Avg. Engagement: {page_data.get('avg_engagement_time', 0):.0f}s"
                    )
        ...
        if kw.get("position") and kw.get("impressions"):
            ...
            print(
                f"  Potential: +{traffic_potential['additional_clicks']} clicks/month (+{traffic_potential['percent_increase']:.0f}%)"
            )
        ...
        if serp_features:
            try:
                intent_result = intent_analyzer.analyze(
                    keyword=keyword, serp_features=serp_features
                )
                ...
                print(
                    f"  Search Intent: {kw['search_intent']} (confidence: {kw['intent_confidence']:.0f}%)"
                )
            except Exception as e:
    ...
    avg_position = sum(k["position"] for k in detailed_opportunities) / len(
        detailed_opportunities
    )
    ...
    for i, kw in enumerate(detailed_opportunities[:3], 1):
        ...
        print(
            f"   Current: {kw['clicks']} clicks → Potential: {int(kw['impressions'] * 0.055)} clicks"
        )
...
def write_markdown_report(opportunities):
    ...
    with open(filename, "w") as f:
        ...
        for i, kw in enumerate(opportunities, 1):
            ...
            f.write(
                f"- **Priority:** {kw.get('enhanced_priority', kw['priority']).upper()}\n"
            )
            ...
            if kw.get("traffic_potential"):
                ...
                f.write(
                    f"- **Gain:** +{tp['additional_clicks']} clicks (+{tp['percent_increase']:.0f}%)\n\n"
                )
